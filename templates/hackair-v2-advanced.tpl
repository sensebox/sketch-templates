...
void setup() {
  ...
  // read config from filesystem
    Serial.print("senseBox ID: ");
    Serial.println(sensebox_id);
    Serial.print("openSenseMap Access Key: ");
...
// Measure data
  ...
  if ( sensebox_id != "" && osem_token != "") {
    ...
    if (client.connect("@@INGRESS_DOMAIN@@", 443)) {
      Serial.println("Connected to @@INGRESS_DOMAIN@@");
      client.print("POST /boxes/");
      client.print(sensebox_id);
      client.print("/data?hackair=true HTTP/1.1\r\n");
      client.print("Host: @@INGRESS_DOMAIN@@\r\n");
      client.print("Connection: close\r\n");
      client.print("Authorization: ");
